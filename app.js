...
function cartQuantityChange(event) {
  ...
  function renderPayments(){
    ...
    // Payment total on dashboard
    if ($("paymentTotal")) {
      $("paymentTotal").textContent = money(totalPaid);
    }

    // Payment total on payment page
    if ($("paymentsPageTotal")) {
      $("paymentsPageTotal").textContent = money(totalPaid);
    }

    const body = $("paymentsBody") || $("paymentsTableBody");
    ...
    body.innerHTML = state.payments.map(p => `
      <tr>
        ...
        <td>
          ${statusPill(p.payment_status || "Pending")}
        </td>
